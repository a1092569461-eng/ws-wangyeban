...
const HL_KEYWORDS_COMPOUND = ['神价', '漏洞', '0元', '免单', '兔单', '速度', '免费'];
const HL_KEYWORDS_SINGLE = ['bug', '速‼', '锁‼', '免', '兔'];
...
function hlMatchKeywords(text) {
  let normalized = hlNormalizeKeyword(text);
  // 将全角感叹号替换为半角感叹号，以便匹配关键词中的半角感叹号
  normalized = normalized.replace(/！/g, '!');
  const found = [];
  for (const kw of HL_KEYWORDS_COMPOUND) {
    if (normalized.includes(kw)) found.push(kw);
  }
  for (const kw of HL_KEYWORDS_SINGLE) {
    if (normalized.includes(kw) && !found.includes(kw)) found.push(kw);
  }
...
function hlIsHighlight(data, allCodes) {
  ...
  // 关键词匹配（基于清理后的文本）
  const keywords = hlMatchKeywords(cleanedText);
  const reasons = [];
  if (keywords.length > 0) reasons.push('keyword');
  
  // 价格提取（基于清理后的文本）
  const price = hlExtractPrice(cleanedText);
  const hasPrice = price !== null;
  
  // 条件3：价格<5且有口令/链接
  if (hasPrice && price < 5 && hasCodeOrLink) {
    reasons.push('price_cheap');
  }
...
function createMsgCard(data, { contentDupInfo = null, productDupInfo = null, productIds = [], isHighlight = false, highlightKeywords = [] } = {}) {
  const card = document.createElement('div');
...
async function addMessage(data){
  ...
  const card = createMsgCard(data, { contentDupInfo: { count: 1, groups: [source] }, productDupInfo: null, productIds: newProductIds, isHighlight: hlResult.triggered, highlightKeywords: hlResult.keywords });
